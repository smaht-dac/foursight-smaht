...
def check_tissue_sample_properties(connection, **kwargs):
    ...
    for tissue_sample in gcc_tissue_samples:
        ...
        tpc_tissue_samples = ff_utils.search_metadata(tpc_search_url,key=connection.ff_keys)
            ...
            for dup_sample in tpc_tissue_samples:
                incorrect.append(
                    {'uuid': dup_sample['uuid'],
                     ...
                     'error': f"Multiple TPC tissue sample items for one external_id {tissue_sample.get('external_id')}"})
        if not tpc_tissue_samples:
            ...
            incorrect.append(
                {'uuid': tissue_sample['uuid'],
                 ...
                 'error': f"No corresponding TPC-submitted tissue sample found for GCC tissue sample with external_id {tissue_sample.get('external_id')}"})
            continue
